...
function      command(){
        ...
        if [ "docker" =  "$provisioner" ] ; then
          ...
          cat ./spec/fixtures/litmus_inventory.yaml | sed -E 's/ [^ :]+:2222:1080/ localhost:1080/g' | sed -E 's/name: ([^:]+:2222)/name: master/g' > ./spec/fixtures/litmus_inventory.yaml.NEW ;
        ...
        echo ;
        echo ;
        (sleep 900 && ssh  -i /tmp/myownkey -A -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -oTCPKeepAlive=yes -oServerAliveInterval=10  -p${deploype_port} -l vagrant ${deploype_ip}  "rm -fr  /tmp/proxy.txt"  ) &
